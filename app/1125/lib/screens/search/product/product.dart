...
class Product extends StatelessWidget {
  ...
  Product({Key key, @required this.itemData, @required this.bookIndex})
      : super(key: key);
  List itemImg;
  ...
  // --- item information ---
  Widget labelInfo({String svgName, String title, Color color}) => ListTile(
        // isThreeLine: true,
        leading: Container(
          width: 24,
          child: Align(
        ...
        subtitle: Text("# ${labelTitle[svgName]}"),
        enabled: false,
        // shape: ,

        // onLongPress: ()=>show,
      );
  Widget itemContent(context, {String title, String text}) {
    return ListView(
      // controller: sc,
      children: [
        Card(
  ...
  Widget build(BuildContext context) {
    ...
    bookInfo = {
      ...
      'date': itemData['info']['date'],
      'pub': itemData['info']['pub'],
      'author': itemData['info']['author'],
      'isbn': itemData['info']['isbn'],
      'page': itemData['info']['page'],
    };
    print(List.generate(itemData['intr'].length, (index) => index * 5));

    _panelHeightOpen = MediaQuery.of(context).size.height * .80;
    itemImg = itemData["imgURL"]
        .map(
          ...
          children: <Widget>[
            ...
            Consumer<SearchModel>(
              builder: (_, localNotify, __) => RawMaterialButton(
                ...
                onPressed: () async{
                  pr.show();
                  var a = await doSearchItem(itemData['info']['isbn']);
                  pr.hide();
                  showSchoolDialog2(context, a);
  ...
  Widget _body(BuildContext context) {
    return Container(
      padding: const EdgeInsets.fromLTRB(0, 15, 0, 130),
      child: ListView(
        children: [
          Card(
              ...
              child: Stack(
                children: [
                  Column(
                    children: [
                      Consumer<SearchModel>(
                        builder: (context, localNotify, _) => Padding(
                          ...
                          child: CarouselSlider(
                              ...
                              items: List.generate(
                                  ...
                                  (index) => GestureDetector(
                                        ...
                                        child: Center(child: itemImg[index]
              ...
              child: Column(
                children: [
                  ...
                  Builder(builder: (context) {
                    ...
                    Map<String, String> snackTitle = {
                      'books': '博客來網路書局',
                      'sanmin': '三民網路書局',
                      'kingstone': '金石堂網路書局',
                      'eslite': '誠品網路書局',
                    };
                    itemData['info']['price'].forEach((k, v) {
                      if (v != null) {
                        priceWidgets.add(_priceField(context,
                            snackTitle: snackTitle[k], storeName: k, price: v));
                      }
                    });
                    ...
                    return Padding(
                      ...
                      child: Column(
                          children: priceWidgets.isNotEmpty
                              ? priceWidgets
                              : [Container()]),
                    );
                  }),
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  Divider(),
                  Column(
                    children: bookInfo.entries.map<Widget>((entry) {
...
// --- Show Dailog ---
  ...
  showSchoolDialog2(BuildContext context, resoult) {
                        ...
                        Row(
                          children: [
                            ...
                            Padding(
                              ...
                              child: Text(
                                // '0',
                                resoult['data'].length==0?'0':resoult['data'][0]['availableItemCountForLoan'],
                                style: Theme.of(context).textTheme.headline2,
                              ),
                            )
                          ],
                        ...
                        Row(
                          children: [
                            ...
                            Padding(
                                ...
                                child: resoult['data'].length==0?Icon(
                                  Icons.clear,
          ...
          children: [
            GestureDetector(
              onTap: () => _showSnack(context,
                  title: snackTitle, icon: Icons.alternate_email),
              child: Container(
                height: 50,
                width: 50,
                decoration: BoxDecoration(
                ...
                child: Image.asset('assets/images/bookStore/$storeName.png'),
              ),
            ),
            if (price == false)
              Icon(Icons.clear)
  ...
  Future doSearchItem(String isbn) async {
    // isbn = '9789863570417';
    var dio = Dio();
    Map resoult = {'statusCode': null, 'data': null};
    final response = await dio
        .get(
          "http://120.108.102.23/inspireapp/API?identify=AUL&func=queryBibliographic&condition=3&input=$isbn&locationId=303&start=0&limit=1&sb=relevance&ob=desc"
        )
        .timeout(const Duration(seconds: 10));
    resoult['statusCode'] = response.statusCode;
    if (response.statusCode == 200) resoult['data'] = json.decode(response.data);
    return resoult;
  }
}
