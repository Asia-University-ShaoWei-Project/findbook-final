...
def ccu(isbn):
  data = None
  url = 'http://webpac.lib.ccu.edu.tw:8888/api/chi/find/TOP02/ISBN={isbn}%20AND%20WFM=(BK%20or%20EB)'.format(isbn = isbn)
  tmp = get_UnivAPI_data(isAPI=True, url=url)
  # --- url2 ---
  if tmp['status'] and ('setno' in tmp['json_data']):
    url = 'http://webpac.lib.ccu.edu.tw:8888/api/chi/fetch/{code}/1/20'.format(code = tmp['json_data']['setno'])
    tmp = get_UnivAPI_data(isAPI=True, url=url)
