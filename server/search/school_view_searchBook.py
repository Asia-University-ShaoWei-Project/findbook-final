...
def index(request):
  if request.method == "POST":
    print('into school')
    req_isbn = request.POST.get('key')
    req_city = request.POST.get('city')
    req_school = request.POST.get('school')
    print("req_isbn: {},req_city: {}, req_school: {}".format(
        req_isbn, req_city, req_school))
    check = checkISBN(req_isbn)
    msg = ''
    if check[0]:  # is ISBN?
      ...
      try:
        ...
        if school_func != None:
          try:
            ...
            # url = url_attr['url'].format(isbn= req_isbn)
            ...
            data = getattr(school_func, req_school)(isbn=req_isbn)
          except:
