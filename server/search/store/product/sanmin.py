...
def sanmin(soup, book_index):
  ...
  if book_model != None:
    ...
    infoData['price'] = {'sanmin': soup.find(
        'span', class_='fs16 Bold fsRed fsale').text.strip()}
  ...
  if tmp:
    ...
    imgURL.extend([i.img['src'].split('big/')[-1]
                  for i in tmp[1:]])  # other img
  else:
    imgURL = [soup.find('img', class_='mainImg')[
        'src'].split('product_images/')[-1]]
  ...
  intrData = [{"title": None, "content": None}
              for i in range(len(tabTitle_find))]
  for index, i in enumerate(tabTitle_find):
    tmp = i.text.strip()  # ex: 商品簡介
    intrData[index]['title'] = tag[tmp] if (
        tmp in tag) else None  # ex: intr, else-> None
  # 內容 ---
  ...
  for indexI, i in enumerate(contents_find):
    ...
    for j in i:
      ...
      word += f'\n{tmp}\n' if tmp != None else "\n".join(
          [k.string for k in j if k.string != None])+'\n'
